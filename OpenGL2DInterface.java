...
public class OpenGL2DInterface {
	...
	public void pushProjection( GL2 gl, int w, int h ) {
		_window_width = w;
		_window_height = h;
		gl.glMatrixMode( GL2.GL_PROJECTION );
		gl.glPushMatrix();
	...
	public void popProjection( GL2 gl ) {
		gl.glMatrixMode( GL2.GL_PROJECTION );
		gl.glPopMatrix();
		gl.glMatrixMode( GL2.GL_MODELVIEW );
		gl.glPopMatrix();
	...
	public void plotPixel( GL2 gl, int x, int y ) {
		y = _window_height - y - 1;
	...
	public void drawRect( GL2 gl, int x, int y, int w, int h ) {
	...
	public void fillRect( GL2 gl, int x, int y, int w, int h ) {
		y = _window_height - y - h;
		gl.glRecti( x, y, x+w, y+h );
	}

	public void drawRectBetweenTwoCorners( GL2 gl, int x1, int y1, int x2, int y2 ) {
		int tmp;
	...
	public void drawCircle( GL2 gl, int x, int y, int radius, boolean filled ) {
		y = _window_height - y - 1;
		if ( filled ) {
